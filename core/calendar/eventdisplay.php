...
$sql = "SELECT d, m, y FROM jelentkezesek WHERE id=" . $id;
...
$sql = "SELECT id, start_time FROM jelentkezesek";
...
function writePosting($id, $auth)
{
	...
	$sql .= "FROM jelentkezesek ";
	$sql .= "LEFT JOIN users ";
	$sql .= "ON ( jelentkezesek.uid = users.uid) ";
	$sql .= "WHERE id = " . $id;
